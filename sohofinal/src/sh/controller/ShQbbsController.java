...
public class ShQbbsController {
	...
	public String Qbbslist(Model model, ShQbbsParam Qparam) { //날려줄 데이터가 있을때 Model model 로
	    ...
	    ArrayList<String> qDateList = new ArrayList<String>();
	      
	    for(int i = 0; i < Qbbslist.size(); i++) {
	        qDateList.add(Qbbslist.get(i).getWdate());
	    }
	    for(int i = 0; i < Qbbslist.size(); i++) {
	         Qbbslist.get(i).setWdate(qDateList.get(i).substring(0, 10));
	    }
	...
	public String Qbbsdetail(ShQbbsDto dto, Model model)throws Exception {
